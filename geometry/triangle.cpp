...
Triangle::Triangle() : Triangle(Point{1, 0, 0}, Point{0, 1, 0}, Point{0, 0, 1}) {}
...
std::vector<Intersection> Triangle::intersects_with(Ray& ray)
{
    ...
    double beta = Matrix::from(3,beta_numerator).determinant() / denominator.determinant();
    ...
    double gamma = Matrix::from(3,gamma_numerator).determinant() / denominator.determinant();
    ...
    if (beta >= 0 && gamma >= 0 && beta + gamma <= 1)
    {
        ...
        double time = Matrix::from(3,time_numerator).determinant() / denominator.determinant();
