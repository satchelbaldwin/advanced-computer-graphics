...
Matrix::Matrix(int n) : size(n), inverse(nullptr), stored_determinant(nullptr) {
...
Matrix::Matrix(Matrix &&m) noexcept
    : data(std::exchange(m.data, nullptr)), size(std::exchange(m.size, 0)),
      inverse(std::exchange(m.inverse, nullptr)),
      stored_determinant(std::exchange(m.stored_determinant, nullptr)) {}

Matrix &Matrix::operator=(const Matrix &m) { return *this = Matrix(m); }
...
bool Matrix::invertable() { return (determinant() != 0); }
...
void Matrix::store_determinant() {
  ...
  if (size == 4) {
    ...
    result = data[0][0] * cofactor(0, 0) + data[1][0] * cofactor(1, 0) +
...
double Matrix::minor(int i, int j) { return submatrix(i, j).determinant(); }
...
double *Matrix::operator[](int i) const { return data[i]; }
