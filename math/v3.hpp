...
public:
  ...
  // multiplication
  template <class T2> friend V3<T> operator*(const V3<T> &v, const T2 &scalar) {
    return V3<T>((T)scalar * v.x, (T)scalar * v.y, (T)scalar * v.z,
                 (T)scalar * v.w);
  }

  template <class T2> friend V3<T> operator*(const T2 &scalar, const V3<T> &v) {
    return V3<T>((T)scalar * v.x, (T)scalar * v.y, (T)scalar * v.z,
                 (T)scalar * v.w);
  }

  // division
  template <class T2> V3<T> operator/(const T2 &scalar) {
  ...
  // dot product
  template <class T2> auto dot(const V3<T2> &v) -> decltype(x * v.x) {
    return (x * v.x + y * v.y + z * v.z);
  }

  // equality
  template <class T2> bool operator==(V3<T2> v) const {
